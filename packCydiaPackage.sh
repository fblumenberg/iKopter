...
echo -e "Can't find the plist: ${PROJECT_NAME}-Info.plist in ${APPMAIN}/Info.plist"
exit 1
fi
#
buildVersion3G=$(/usr/libexec/PlistBuddy -c "Print CFBundleVersion" "${buildPlist}" 2>/dev/null)
if [[ "${buildVersion}" = "" ]]
then
echo -e "\"${buildPlist}\" does not contain key: \"CFBundleVersion\""
exit 1
...
cd ${PROJECTMAIN}/Cydia/iKopter3G/DEBIAN
sed 's/Version:.*$/Version: '${buildVersion3G}'/' control > control1
echo "==============================="
mv control1 control
