...
#import "IASKPotiTextFieldSpecifierViewCell.h"
#import "IASKSwitch.h"
#import "IASKSlider.h"
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  if ([[specifier type] isEqualToString:kIASKPSToggleSwitchSpecifier]) {
  ...
  else if ([[specifier type] isEqualToString:kIASKPotiTextFieldSpecifier]) {
    IASKPotiTextFieldSpecifierViewCell *cell = (IASKPotiTextFieldSpecifierViewCell*)[tableView dequeueReusableCellWithIdentifier:[specifier type]];
    
    if (!cell) {
      cell = (IASKPotiTextFieldSpecifierViewCell*) [[[NSBundle mainBundle] loadNibNamed:@"IASKPotiTextFieldSpecifierViewCell" 
                                                                                  owner:self 
                                                                                options:nil] objectAtIndex:0];
    ...
    NSNumber* value = [self.settingsStore objectForKey:key] != nil ? [self.settingsStore objectForKey:key] : [specifier defaultValue];
    [cell textField].value=[value intValue];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  if ([[specifier type] isEqualToString:kIASKPSToggleSwitchSpecifier]) {
    ...
    [tableView deselectRowAtIndexPath:indexPath animated:NO];
		...
		IASKPotiTextFieldSpecifierViewCell *textFieldCell = (id)[tableView cellForRowAtIndexPath:indexPath];
		[textFieldCell.textField becomeFirstResponder];
  }
  else if ([[specifier type] isEqualToString:kIASKPSChildPaneSpecifier]) {
    ...
    if ([MFMailComposeViewController canSendMail]) {
      ...
      if ([specifier localizedObjectForKey:kIASKMailComposeAttachment]) {
        
        if ([self.delegate respondsToSelector:@selector(mailComposeAttachment:)]) {
          [self.delegate mailComposeAttachment:mailViewController];
        }
      }
...
- (void)_textChanged:(id)sender {
  ...
  IASKSpecifier *spec   = [_settingsReader specifierForKey:[text key]];
  ...
  [_settingsStore setObject:value forKey:[text key]];
  [[NSNotificationCenter defaultCenter] postNotificationName:kIASKAppSettingChanged
                                                      object:[text key]
